...
<body>
	...
	MemberDto memberDto = new MemberDto(); // dto 객체  필드값 초기화
	...
	//	String sid = null;
			...
			if(rs.next()){ // 참이면 레코드가 1개 이상 존재 -> 아이디가 존재
				do { // rs에서 레코드를 추출하는 방법 hasnext
					...
					memberDto.setMemberedate(sdate);
	...
	<input type="hidden" name="mid" value="${memberDto.memberid}" >	
		아이디 : <br><input type="text" name="mid" value="${memberDto.memberid}" disabled="disabled"><br>
		비밀번호 :<br> <input type="password" name="mpw" value="${memberDto.memberpw}"> <br>
		이름 : <br><input type="text" name="mname" value="${memberDto.membername}"> <br>
		이메일 : <br><input type="text" name="memail" value="${memberDto.memberemail}"> <br>
		가입일 : <br><input type="text" name="mdate" value="${memberDto.memberdate}" readonly="readonly"> <br>
